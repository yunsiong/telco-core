namespace Telco {
	...
	private class ResourceStore {
		...
		public ResourceStore (TemporaryDirectory tempdir) throws Error {
			...
			var blob = Telco.Data.Helper.get_telco_helper_blob ();
			helper = new TemporaryHelperFile (
				new TemporaryFile.from_stream ("telco-helper",
					new MemoryInputStream.from_data (blob.data, null),
					tempdir));
			FileUtils.chmod (helper.path, 0700);
#else
			helper = new InstalledHelperFile.for_path (Config.TELCO_HELPER_PATH);
#endif
		}
