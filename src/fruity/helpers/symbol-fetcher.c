...
static void telco_append_string (char ** output, const char * val);
static void telco_append_char (char ** output, char val);
static void telco_append_uint64 (char ** output, uint64_t val);
...
static bool telco_str_contains (const char * str, const char * needle);
static const char * telco_strstr (const char * str, const char * needle);

size_t
telco_fetch_dyld_symbols (char * output_buffer, const void * dyld_load_address)
{
  TelcoMachO dyld;
  size_t size;
  ...
  telco_parse_macho (dyld_load_address, &dyld);
  ...
  for (i = dyld.dysymtab->ilocalsym; i != dyld.dysymtab->nlocalsym; i++)
  {
    ...
    if (telco_str_contains (name, "libdyld_initialize") ||
        telco_str_contains (name, "restartWithDyldInCache") ||
        telco_str_equals (name, "_gProcessInfo") ||
        telco_str_equals (name, "__ZN5dyld412gProcessInfoE") ||
        telco_str_contains (name, "launchWithClosure") ||
        telco_str_contains (name, "initializeMainExecutable") ||
        telco_str_contains (name, "registerThreadHelpers") ||
        telco_str_has_prefix (name, "_dlopen") ||
        telco_str_has_prefix (name, "_strcmp") ||
        telco_str_contains (name, "doModInitFunctions") ||
        telco_str_contains (name, "doGetDOFSections"))
    {
      if (n != 0)
        telco_append_char (&cursor, '\n');

      telco_append_uint64 (&cursor, (uint64_t) (dyld.base + sym->n_value));
      telco_append_char (&cursor, '\t');
      telco_append_string (&cursor, name);
  ...
  telco_append_char (&cursor, '\n');
  telco_append_uint64 (&cursor, dyld.size);
  telco_append_char (&cursor, '\t');
  telco_append_string (&cursor, "dyld_size");

  size = cursor - output_buffer;

  telco_append_char (&cursor, '\0');
...
static void
telco_append_string (char ** output, const char * val)
{
  char * cursor = *output;
  char c;
...
static void
telco_append_char (char ** output, char val)
{
  char * cursor = *output;
...
static void
telco_append_uint64 (char ** output, uint64_t val)
{
...
static bool
telco_str_contains (const char * str, const char * needle)
{
  return telco_strstr (str, needle) != NULL;
}

static const char *
telco_strstr (const char * str, const char * needle)
{
  char first, c;
  ...
  while ((c = *str) != '\0')
  {
    if (c == first && telco_str_has_prefix (str, needle))
      return str;
    str++;
  }
...
main (void)
{
  ...
  size = telco_fetch_dyld_symbols (output_buffer, dyld_info->dyldImageLoadAddress);
