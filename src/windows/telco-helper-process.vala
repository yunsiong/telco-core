namespace Telco {
	...
	private class HelperFactory {
		...
		public async HelperInstance obtain (Cancellable? cancellable) throws Error, IOError {
			...
			if (obtain_request == null) {
				...
				new Thread<bool> ("telco-helper-factory", obtain_worker);
			}
	...
	private class ResourceStore {
		...
		public ResourceStore (TemporaryDirectory tempdir) throws Error {
			var blob32 = Telco.Data.Helper.get_telco_helper_32_exe_blob ();
			helper32 = new TemporaryFile.from_stream ("telco-helper-32.exe",
				new MemoryInputStream.from_data (blob32.data, null),
				tempdir);

			var blob64 = Telco.Data.Helper.get_telco_helper_64_exe_blob ();
			helper64 = new TemporaryFile.from_stream ("telco-helper-64.exe",
