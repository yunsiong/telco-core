...
typedef struct _TelcoInjectionDetails TelcoInjectionDetails;
typedef struct _TelcoRemoteWorkerContext TelcoRemoteWorkerContext;
...
struct _TelcoRemoteWorkerContext
{
...
static void telco_propagate_open_process_error (guint32 pid, DWORD os_error, GError ** error);
static gboolean telco_enable_debug_privilege (void);

static gboolean telco_remote_worker_context_init (TelcoRemoteWorkerContext * rwc, TelcoInjectionDetails * details, GError ** error);
static void telco_remote_worker_context_destroy (TelcoRemoteWorkerContext * rwc, TelcoInjectionDetails * details);

static gboolean telco_remote_worker_context_has_resolved_all_kernel32_functions (const TelcoRemoteWorkerContext * rwc);
static gboolean telco_remote_worker_context_collect_kernel32_export (const GumExportDetails * details, gpointer user_data);

static gboolean telco_file_exists_and_is_readable (const WCHAR * filename);

void
_telco_windows_helper_backend_inject_library_file (guint32 pid, const gchar * path, const gchar * entrypoint, const gchar * data,
    void ** inject_instance, void ** waitable_thread_handle, GError ** error)
{
  gboolean success = FALSE;
  const gchar * failed_operation;
  NTSTATUS nt_status;
  TelcoInjectionDetails details;
  DWORD desired_access;
  HANDLE thread_handle = NULL;
  gboolean rwc_initialized = FALSE;
  TelcoRemoteWorkerContext rwc;
  ...
  if (!telco_file_exists_and_is_readable (details.dll_path))
    goto invalid_path;

  telco_enable_debug_privilege ();
  ...
  if (!telco_remote_worker_context_init (&rwc, &details, error))
    goto beach;
  ...
  instance->stay_resident_address = (guint8 *) rwc.argument + G_STRUCT_OFFSET (TelcoRemoteWorkerContext, stay_resident);
...
os_failure:
  {
    ...
    {
      telco_propagate_open_process_error (pid, os_error, error);
    }
    else
    {
      g_set_error (error,
          TELCO_ERROR,
          (os_error == ERROR_ACCESS_DENIED) ? TELCO_ERROR_PERMISSION_DENIED : TELCO_ERROR_NOT_SUPPORTED,
...
nt_failure:
  {
    ...
    if (nt_status == 0xC0000022) /* STATUS_ACCESS_DENIED */
      code = TELCO_ERROR_PERMISSION_DENIED;
    else
      code = TELCO_ERROR_NOT_SUPPORTED;
...
beach:
  {
    if (!success && rwc_initialized)
      telco_remote_worker_context_destroy (&rwc, &details);
...
void
_telco_windows_helper_backend_free_inject_instance (void * inject_instance, gboolean * is_resident)
{
  TelcoInjectInstance * instance = inject_instance;
  gboolean stay_resident;
  SIZE_T n_bytes_read;
...
static void
telco_propagate_open_process_error (guint32 pid, DWORD os_error, GError ** error)
{
...
static gboolean
telco_enable_debug_privilege (void)
{
...
static gboolean
telco_remote_worker_context_init (TelcoRemoteWorkerContext * rwc, TelcoInjectionDetails * details, GError ** error)
{
  gpointer code;
  guint code_size;
  GumX86Writer cw;
  ...
  /* xbx = (TelcoRemoteWorkerContext *) lpParameter */
  ...
  gum_x86_writer_put_lea_reg_reg_offset (&cw, GUM_X86_XCX,
      GUM_X86_XBX, G_STRUCT_OFFSET (TelcoRemoteWorkerContext, dll_path));
  gum_x86_writer_put_call_reg_offset_ptr_with_arguments (&cw, GUM_CALL_SYSAPI,
      GUM_X86_XBX, G_STRUCT_OFFSET (TelcoRemoteWorkerContext, load_library_impl),
      1,
  ...
  gum_x86_writer_put_lea_reg_reg_offset (&cw, GUM_X86_XDX,
      GUM_X86_XBX, G_STRUCT_OFFSET (TelcoRemoteWorkerContext, entrypoint_name));
  gum_x86_writer_put_call_reg_offset_ptr_with_arguments (&cw, GUM_CALL_SYSAPI,
      GUM_X86_XBX, G_STRUCT_OFFSET (TelcoRemoteWorkerContext, get_proc_address_impl),
      2,
  ...
  gum_x86_writer_put_lea_reg_reg_offset (&cw, GUM_X86_XCX,
      GUM_X86_XBX, G_STRUCT_OFFSET (TelcoRemoteWorkerContext, entrypoint_data));
  gum_x86_writer_put_lea_reg_reg_offset (&cw, GUM_X86_XDX,
      GUM_X86_XBX, G_STRUCT_OFFSET (TelcoRemoteWorkerContext, stay_resident));
  ...
  /* FreeLibrary (xsi) */
  gum_x86_writer_put_call_reg_offset_ptr_with_arguments (&cw, GUM_CALL_SYSAPI,
      GUM_X86_XBX, G_STRUCT_OFFSET (TelcoRemoteWorkerContext, free_library_impl),
      1,
      GUM_ARG_REGISTER, GUM_X86_XSI);
  ...
  /* result = GetLastError() */
  gum_x86_writer_put_call_reg_offset_ptr_with_arguments (&cw, GUM_CALL_SYSAPI,
      GUM_X86_XBX, G_STRUCT_OFFSET (TelcoRemoteWorkerContext, get_last_error_impl),
      0);
  ...
  memset (rwc, 0, sizeof (TelcoRemoteWorkerContext));

  gum_module_enumerate_exports ("kernel32.dll", telco_remote_worker_context_collect_kernel32_export, rwc);
  if (!telco_remote_worker_context_has_resolved_all_kernel32_functions (rwc))
  ...
  rwc->entrypoint = VirtualAllocEx (details->process_handle, NULL,
      code_size + data_alignment + sizeof (TelcoRemoteWorkerContext), MEM_COMMIT, PAGE_EXECUTE_READWRITE);
  ...
  if (!WriteProcessMemory (details->process_handle, rwc->argument, rwc, sizeof (TelcoRemoteWorkerContext), NULL))
    goto write_process_memory_failed;
...
static void
telco_remote_worker_context_destroy (TelcoRemoteWorkerContext * rwc, TelcoInjectionDetails * details)
{
  if (rwc->entrypoint != NULL)
  {
...
static gboolean
telco_remote_worker_context_has_resolved_all_kernel32_functions (const TelcoRemoteWorkerContext * rwc)
{
...
static gboolean
telco_remote_worker_context_collect_kernel32_export (const GumExportDetails * details, gpointer user_data)
{
  TelcoRemoteWorkerContext * rwc = user_data;
...
gboolean
telco_windows_system_is_x64 (void)
{
...
gboolean
telco_windows_process_is_x64 (guint32 pid, GError ** error)
{
  HANDLE process_handle;
...
open_failed:
  {
    telco_propagate_open_process_error (pid, GetLastError (), error);
    return FALSE;
  }
query_failed:
  {
...
static gboolean
telco_file_exists_and_is_readable (const WCHAR * filename)
{
  HANDLE file;
