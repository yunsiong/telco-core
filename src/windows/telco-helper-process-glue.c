#include "telco-helper-process-glue.h"
...
void *
telco_helper_factory_spawn (const gchar * path, const gchar * parameters, TelcoPrivilegeLevel level, GError ** error)
{
  ...
  if (level == TELCO_PRIVILEGE_LEVEL_ELEVATED)
    ei.lpVerb = L"runas";
  else
    ei.lpVerb = L"open";
...
gboolean
telco_helper_instance_is_process_still_running (void * handle)
{
  DWORD exit_code;
...
void
telco_helper_instance_close_process_handle (void * handle)
{
