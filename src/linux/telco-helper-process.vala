namespace Telco {
	...
	private class HelperFactory {
		...
		public async LinuxHelper obtain (Cancellable? cancellable) throws Error, IOError {
			...
			try {
				string socket_path = "/telco-" + Uuid.string_random ();
	...
	private class ResourceStore {
		...
		public ResourceStore (TemporaryDirectory tempdir) throws Error {
			...
			var blob32 = Telco.Data.Helper.get_telco_helper_32_blob ();
			if (blob32.data.length > 0)
				helper32 = make_temporary_helper ("telco-helper-32", blob32.data);

			var blob64 = Telco.Data.Helper.get_telco_helper_64_blob ();
			if (blob64.data.length > 0)
				helper64 = make_temporary_helper ("telco-helper-64", blob64.data);
#else
			var tpl = PathTemplate (Config.TELCO_HELPER_PATH);
